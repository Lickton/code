...
void do_bgfg(char **argv) 
{
    ...
    if (argv[1][0] == '%') {
        for (int i = 1; i < n; i++) {
            ...
            tmp_jid = tmp_jid * 10 + (argv[1][i] - '0');
        }
        if (getjobjid(jobs, tmp_jid) != NULL) {
            tmp_pid = getjobjid(jobs, tmp_jid)->pid;
        } else {
        ...
        for (int i = 0; i < n; i++) {
            ...
            tmp_pid = tmp_pid * 10 + (argv[1][i] - '0');
        }
        if ((tmp_jid = pid2jid(tmp_pid)) == 0) {
    ...
    struct job_t *tmp = getjobjid(jobs, tmp_jid);
    if (!strcmp(argv[0], "fg")) {
        tmp->state = FG;
        ...
        tmp->state = BG;
        kill(-tmp_pid, SIGCONT);
        printf("[%d] (%d) %s", tmp_jid, tmp_pid, tmp->cmdline);
    }
    return;
}
...
void sigchld_handler(int sig)
{
    ...
    while (waitpid(-1, &status, WNOHANG | WUNTRACED) > 0) {
        if (WIFEXITED(status)) {
            ...
            if (verbose)
                printf("sigchld_handler: Job [%d] (%d) deleted\n", fg_jid, fg_pid);
            printf("Job [%d] (%d) terminated by signal %d\n", fg_jid, fg_pid, WTERMSIG(status));
        } else if (WIFSTOPPED(status)) {
            sigprocmask(SIG_BLOCK, &mask_all, &prev);
            getjobpid(jobs, fg_pid)->state = ST;
            printf("Job [%d] (%d) stopped by signal %d\n", fg_jid, fg_pid, WSTOPSIG(status));
            sigprocmask(SIG_SETMASK, &prev, NULL);
        }
    }
